...
void computeAngleMT(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
  ...
  std::for_each(std::execution::par_unseq, keypoints.begin(), keypoints.end(),
                [&half_patch_size, &image](auto &kp) {
                  // Check if the keypoint is close to the edges
                  if (kp.pt.x >= half_patch_size*2 && kp.pt.y >= half_patch_size*2 &&
                    kp.pt.x < image.cols - half_patch_size*2 && kp.pt.y < image.rows - half_patch_size*2){
                    // Compute angles
                    float m01 = 0, m10 = 0;
                    for (int dx = -half_patch_size; dx < half_patch_size; ++dx) {
                      for (int dy = -half_patch_size; dy < half_patch_size; ++dy) {
...
void computeORBDescMT(const cv::Mat &image, vector<cv::KeyPoint> &keypoints,
  ...
  // Compute number of bad descriptors
  int bad = 0;
  for (auto &d : desc) {
    if (d.empty())
      bad++;
